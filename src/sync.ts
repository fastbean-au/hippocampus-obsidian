...
import { isUnder, resolveGroup, resolveSignificance, stripFrontmatter } from "./mapping";
...
export class SyncEngine {
	...
	async syncFile(file: TFile): Promise<void> {
		...
		const body = settings.stripFrontmatter ? stripFrontmatter(content) : content;
		...
		try {
			if (knownId !== undefined) {
				await this.plugin.client.updateMemory(knownId, body, significance);

				return;
			}

			await this.store(file.path, body, significance, group);
		} catch (err) {
	...
	private async store(path: string, body: string, significance: number, group: string): Promise<void> {
		const result = await this.plugin.client.storeMemory({ body, significance, group });
	...
	async syncAll(): Promise<{ synced: number; failed: number }> {
		const files = this.plugin.app.vault.getMarkdownFiles().filter((file) => this.isSyncable(file));
		let synced = 0;
		let failed = 0;
		...
		new Notice(`Hippocampus: synced ${synced} note(s)` + (failed > 0 ? `, ${failed} failed` : ""));
