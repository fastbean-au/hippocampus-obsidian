...
export class HippocampusSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		new Setting(containerEl)
			.setName("Server URL")
			.setDesc("Base URL of the Hippocampus HTTP gateway (requires gateway.port > 0 on the service).")
			.addText((text) =>
				text
		...
		new Setting(containerEl)
			...
			.setDesc("Sent as 'Authorization: Bearer <token>' when the service requires auth. Leave empty otherwise.")
			.addText((text) => {
				text
					.setPlaceholder("(none)")
					.setValue(s.token)
		...
		new Setting(containerEl).addButton((btn) =>
			btn.setButtonText("Test connection").onClick(() => this.plugin.testConnection()),
		);
		...
		new Setting(containerEl)
			...
			.setDesc("Significance used when a note has no significance frontmatter. Higher survives longer.")
			.addText((text) =>
		...
		new Setting(containerEl)
			...
			.setDesc("A note's frontmatter value under this key overrides the default significance.")
			.addText((text) =>
		...
		new Setting(containerEl)
			...
			.setDesc("Remove a note's leading YAML frontmatter before storing it as a memory body.")
			.addToggle((toggle) =>
		...
		new Setting(containerEl)
			...
			.setDesc("Recall (reinforce) matched memories when searching, resetting their decay clock.")
			.addToggle((toggle) =>
		...
		new Setting(containerEl)
			...
			.setDesc("Push notes under the sync folder into Hippocampus as they are edited, keeping each note's memory up to date.")
			.addToggle((toggle) =>
		...
		new Setting(containerEl)
			.setName("Sync folder")
			.setDesc("Vault-relative folder whose notes are synced. Empty means the whole vault.")
			.addText((text) =>
