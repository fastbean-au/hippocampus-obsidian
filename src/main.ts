...
export default class HippocampusPlugin extends Plugin {
  ...
  async onload(): Promise<void> {
    ...
    this.registerEvent(
      this.app.vault.on("modify", (file) => this.sync.onModify(file)),
    );
    this.registerEvent(
    ...
    this.registerEvent(
      this.app.vault.on("delete", (file) => this.sync.onDelete(file)),
    );
  }
  ...
  private async storeSelection(editor: Editor): Promise<void> {
    ...
    const group = file
      ? resolveGroup(this.settings, file.path, frontmatter)
      : this.settings.groupFixedValue;
    const metadata = resolveMetadata(this.settings, frontmatter);

    try {
      const result = await this.client.storeMemory({
        body: this.settings.stripFrontmatter
          ? stripFrontmatter(selection)
          : selection,
        significance,
        group,
        metadata,
...
function formatSearchResults(query: string, memories: MemoryView[]): string {
  ...
  const lines = memories.map((memory) => {
    const meta =
      ...
      (memory.group ? `, group ${memory.group}` : "");
